            ...
            <form onSubmit={handleSubmit} className="space-y-6 select-text">
              <div className="space-y-2.5 text-center pb-2">
                <h3 className="text-xl md:text-2xl font-semibold tracking-tight text-white leading-tight font-sans">
                  {isLoginMode ? "Sign In to Your Account" : "Get a Free Quote from Korea's Master Tailor ✂️"}
                </h3>
              ...
              <div className="pt-2 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
                ...
                <button
                  type="submit"
                  disabled={isLoginMode ? (!email.trim() || !password.trim()) : (!email.trim() || !country.trim() || !password.trim() || !checked)}
                  className={`px-6 py-3 rounded-full text-xs font-semibold tracking-wider uppercase transition-all duration-300 flex items-center justify-center space-x-2 shrink-0 ${
                    (isLoginMode ? (email.trim() && password.trim()) : (email.trim() && country.trim() && password.trim() && checked))
              ...
              <div className="text-center pt-2">
                <button
                  ...
                  className="text-neutral-500 text-[11px] font-sans hover:text-amber-400 transition-colors"
                >
                  {isLoginMode 
